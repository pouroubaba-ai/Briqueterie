...
import {
  ...
  BarChart2, Factory, Building2, CreditCard,
} from "lucide-react";

const links = [
  { href: "/", icon: LayoutDashboard, label: "Tableau de bord" },
  { href: "/stock", icon: Package, label: "Inventaire" },
...
export default function BurgerNav() {
  ...
  return (
    <>
      {/* Barre du haut avec burger */}
      <div className="fixed top-0 left-0 right-0 z-40 bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-lg mx-auto flex items-center gap-3 px-4 h-14">
          <button
            onClick={() => setOpen(true)}
            className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-gray-50 text-gray-600"
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-800">Briqueterie</span>
          </div>
        </div>
      </div>
          ...
          <div className="absolute left-0 top-0 bottom-0 w-72 bg-white shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-5 py-5 border-b border-gray-100">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-base font-bold text-gray-900">Briqueterie</span>
              </div>
            ...
            <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
              ...
              <div>
                ...
                {finOpen && (
                  <div className="ml-4 mt-1 space-y-0.5">
                    {financesLinks.map(({ href, icon: Icon, label }) => {
              ...
              <div>
                ...
                {commOpen && (
                  <div className="ml-4 mt-1 space-y-0.5">
                    {commercialLinks.map(({ href, icon: Icon, label }) => {
                      const active = pathname.startsWith(href);
                      return (
                        <Link key={href} href={href} onClick={() => setOpen(false)}
                          className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-colors ${
                ...
                {histOpen && (
                  <div className="ml-4 mt-1 space-y-0.5">
                    <Link href="/historique?type=sortie" onClick={() => setOpen(false)}
                      className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-colors ${
                        pathname === "/historique" && typeof window !== "undefined" && new URLSearchParams(window.location.search).get("type") === "sortie"
                    ...
                    <Link href="/historique?type=entree" onClick={() => setOpen(false)}
                      className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-colors ${
                        pathname === "/historique" && typeof window !== "undefined" && new URLSearchParams(window.location.search).get("type") === "entree"
