...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    for (const b of prod.briques) {
      // Retirer les briques produites du stock
      ...
      // Remettre le ciment consommé en stock si c'est du ciment
      if (b.brique.estCiment && b.nombreSacs > 0) {
        await prisma.brique.update({ where: { id: b.briqueId }, data: { stockActuel: { increment: b.nombreSacs } } });
      }
    }
