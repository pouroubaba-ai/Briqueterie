...
export async function GET(req: NextRequest) {
  ...
  for (const s of sorties) {
    mouvements.push({ id: `sortie-${s.id}`, type: "sortie", categorie: s.type, date: s.date.toISOString(), source: s.notes || s.type, notes: s.notes, produits: [{ briqueId: s.briqueId, nom: s.brique.nom, dimensions: s.brique.dimensions, quantite: s.quantite, unite: "unités" }] });
  }
