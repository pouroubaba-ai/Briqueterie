...
function granulariteAuto(de: Date, a: Date) {
  ...
  if (diff <= 1)   return "heure";   // aujourd'hui → par heure
  if (diff <= 8)   return "jour";    // jusqu'à 1 semaine → par jour
  return "mois";                     // ce mois, cette année, custom → par mois
}

async function getCaDepenses(de: Date, a: Date) {
  const [factures, productions, versements, depenses] = await Promise.all([
    prisma.facture.findMany({ include: { paiements: true }, where: { createdAt: { gte: de, lt: a } } }),
    prisma.productionJour.findMany({ where: { date: { gte: de, lt: a } } }),
    prisma.versementFournisseur.findMany({ where: { date: { gte: de, lt: a } } }),
    prisma.depense.findMany({ where: { date: { gte: de, lt: a } } }),
  ]);
  const ca = factures.reduce((s, f) => s + f.paiements.reduce((ps, p) => ps + p.montant, 0), 0);
  const dep = productions.reduce((s, p) => s + p.montantVerse, 0)
    + versements.reduce((s, v) => s + v.montant, 0)
...
export async function GET(req: Request) {
  ...
  const [briques, factures, sorties, productions, versementsFourn, depensesDiverses, tousLesAchats, tousLesVersementsFourn] = await Promise.all([
    ...
    prisma.sortieStock.findMany({
      where: { date: { gte: debut, lte: fin }, type: { notIn: ["production", "annulation_production"] } },
      include: { brique: true },
    }),
    prisma.productionJour.findMany({
      where: { date: { gte: debut, lte: fin } },
      include: { briques: { include: { brique: true } } },
    }),
    prisma.versementFournisseur.findMany({ where: { date: { gte: debut, lte: fin } } }),
    prisma.depense.findMany({ where: { date: { gte: debut, lte: fin } } }),
    prisma.achatFournisseur.findMany({ include: { fournisseur: true } }),
    prisma.versementFournisseur.findMany(),
  ]);
  ...
  const totalFacture = facturesPeriode.reduce((s, f) =>
    s + f.livraison.lignes.reduce((ls, l) => ls + l.quantiteLivree * l.brique.prixVente, 0) + f.transport, 0);
  ...
  if (granularite === "heure") {
    for (let h = 0; h < 24; h++) {
      ...
      const { ca, dep } = await getCaDepenses(deb, fin2);
    ...
    const startYear = debut.getFullYear();
    const endYear = fin.getFullYear();
    for (let y = startYear; y <= endYear; y++) {
      const deb = new Date(y, 0, 1);
      const fin2 = new Date(y + 1, 0, 1);
      const { ca, dep } = await getCaDepenses(deb, fin2);
      historique.push({ label: String(y), ca, depenses: dep });
    }
  }
